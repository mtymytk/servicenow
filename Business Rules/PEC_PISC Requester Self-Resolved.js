(function executeRule(current, previous /*null when async*/) {
    //PISCインスタンスのu_support_desk_inquiryテーブルのレコードにて起票者が自己解決する
	var action = "update";
    ...
    while (successfulResponceStatus == false && count > 0) {
		...
		//requestBody.u_closed = current.resolved_at.toString(); //クローズ日時
		requestBody.u_description_dummy = "自己解決のため、クローズ"; //
